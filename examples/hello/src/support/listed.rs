use super::{i32_to_usize, Error, Result};
use std::{
    cell::RefCell,
    ops::Deref,
    sync::{Arc, RwLock},
    ...
    T: Clone + Sync + Send,
{
    ...
    fn get(&self, index: i32) -> Result<T>;
    ...
    T: Clone + Sync + Send,
{
    ...
    pub fn join(
        &self,
        separator: impl crate::support::ToArcString,
...
mod listed {
    use super::{List, ListedTrait, Result};
    use crate::support::ToArcString;
        ...
        T: Clone + Sync + Send,
    {
        ...
        for i in 0..listed.len() {
            // TODO An unchecked get?
            let value = listed.get(i).unwrap();
        ...
        T: Clone + Sync + Send,
    {
        ...
        for i in 0..listed.len() {
            ...
            // TODO An unchecked get?
            result.push_str(&stringify(listed.get(i).unwrap()));
        }
        ...
        T: Clone + Sync + Send,
    {
        ...
        for i in 0..listed.len() {
            // TODO An unchecked get?
            result.push(transform(listed.get(i).unwrap()));
        }
        Arc::new(result)
    }

    pub fn reduce<T>(listed: &dyn ListedTrait<T>, accumulate: &dyn Fn(T, T) -> T) -> Result<T>
    where
        T: Clone + Sync + Send,
    {
        Ok(reduce_from_index(listed, listed.get(0)?, 1, accumulate))
    }
        ...
        O: Clone + Sync + Send,
    {
        ...
        for i in index..listed.len() {
            // TODO An unchecked get?
            result = accumulate(result, listed.get(i).unwrap());
        }
        result
    }
    ...
    T: 'static + Clone + Sync + Send,
{
    ...
    fn get(&self, index: i32) -> Result<T> {
        Ok(<[T]>::get(self, i32_to_usize(index)?)
...
pub mod list_builder {
    ...
    // TODO We'd want pub if separate temper_core crate.
    // TODO pub(crate) lots of things?

    pub(crate) fn add<T>(builder: &ListBuilder<T>, value: T, index: Option<i32>) -> Result<()> {
        match index {
            Some(index) => {
    ...
    pub(crate) fn add_all<T>(
        builder: &ListBuilder<T>,
        values: impl ToListed<T>,
        index: Option<i32>,
    ) -> Result<()>
    where
        T: Clone + Sync + Send,
    ...
    pub fn remove_last<T>(builder: &ListBuilder<T>) -> Result<T> {
        builder.write().unwrap().pop().ok_or_else(Error::new)
    }
    ...
    T: 'static + Clone + Sync + Send,
{
    ...
    fn get(&self, index: i32) -> Result<T> {
        Ok(<[T]>::get(&self.read().unwrap(), i32_to_usize(index)?)
            .ok_or_else(Error::new)?
            .clone())
    }
